...
class OneEuro:
    def __init__(self, freq=30.0, min_cutoff=0.60, beta=0.05, dcutoff=0.8):
...
class TemporalMedianFilter:
    """Remove outlier spikes by taking median of last N frames"""
    def __init__(self, window_size=5):
        from collections import deque
        self.window_size = window_size
        self.windows = {}

    def __call__(self, values_dict):
        """values_dict: {name: angle} for all fingers"""
        result = {}
        for name, value in values_dict.items():
            ...
            result[name] = float(np.median(list(self.windows[name])))
        return result


class RateLimiter:
    """Limit maximum change per frame to prevent sudden jumps"""
    def __init__(self, max_change=6.0):
        ...
        for name, value in values_dict.items():
            if name not in self.prev_values:
                ...
                if abs(delta) > self.max_change:
                    result[name] = prev + np.sign(delta) * self.max_change
                else:
                    result[name] = value
                self.prev_values[name] = result[name]
        return result


class HysteresisFilter:
    """Only update if change exceeds threshold - prevents micro-jitter"""
    ...
    def __call__(self, values_dict):
        ...
        for name, value in values_dict.items():
            if name not in self.prev_values:
                ...
                prev = self.prev_values[name]
                if abs(value - prev) < self.threshold:
                    result[name] = prev  # Keep old value
                else:
                    result[name] = value
                    self.prev_values[name] = value
        return result
...
def parse_args():
    ...
    parser.add_argument("--min-cutoff", type=float, default=0.80)
    parser.add_argument("--beta", type=float, default=0.04)
    parser.add_argument("--dcutoff", type=float, default=1.0)
    parser.add_argument("--median-window", type=int, default=10, help="Temporal median filter window size")
    parser.add_argument("--rate-limit", type=float, default=10.0, help="Max angle change per frame (degrees)")
    parser.add_argument("--hysteresis", type=float, default=1.0, help="Dead zone threshold (degrees)")
    parser.add_argument("--enable-median", action="store_true", default=True, help="Enable temporal median filter")
    parser.add_argument("--enable-rate-limit", action="store_true", default=True, help="Enable rate limiter")
    parser.add_argument("--enable-hysteresis", action="store_true", default=True, help="Enable hysteresis filter")
...
def main():
    ...
    # Initialize anti-jitter filters
    if args.enable_median:
        median_filter = TemporalMedianFilter(window_size=args.median_window)
    if args.enable_rate_limit:
        rate_limiter = RateLimiter(max_change=args.rate_limit)
    if args.enable_hysteresis:
        hysteresis_filter = HysteresisFilter(threshold=args.hysteresis)
    ...
    while True:
        ...
        if finger_angles:
            ...
            # Build dict for additional anti-jitter filters
            angle_dict = {
                "THUMB": thumb_a,
                "INDEX": index_a,
                "MIDDLE": middle_a,
            ...
            # Apply cascaded anti-jitter filters
            if median_filter is not None:
                angle_dict = median_filter(angle_dict)
            if rate_limiter is not None:
                angle_dict = rate_limiter(angle_dict)
            if hysteresis_filter is not None:
                angle_dict = hysteresis_filter(angle_dict)
            
            # Extract final filtered angles
            thumb_a = angle_dict["THUMB"]
            index_a = angle_dict["INDEX"]
            middle_a = angle_dict["MIDDLE"]
            ring_a = angle_dict["RING"]
            pinky_a = angle_dict["PINKY"]
            ...
            t_now = time.time()
            # With cascaded filters, send on any detected change or keepalive
            should_send = (
